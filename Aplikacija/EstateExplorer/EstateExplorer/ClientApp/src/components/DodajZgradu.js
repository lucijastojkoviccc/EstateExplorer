...
export default function DodajZgradu() {
    ...
    useEffect(() => {
        ...
        if (state && state.id !== null && state.id !== undefined) {
            axios.get(`http://localhost:3000/zgrade/${state.id}`)
                .then(p => { console.log(p.data); setZgrada(p.data) })
    ...
    const handleSubmit = (event) => {
        ...
        let req;
        if (zgrada.id >= 0) {
            req = axios.put(`http://localhost:3000/zgrade/${zgrada.id}`, zgrada);
        }
        else {
            req = axios.post("http://localhost:3000/zgrade", zgrada)
        }
        req.then(p => {
