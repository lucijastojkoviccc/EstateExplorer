...
export function RatePrikaz()
{
    ...
    useEffect(() => {
        axios.get("http://localhost:3000/nekretnine")
        .then(res => {
    ...
    useEffect(() => {
        (async () => {
            ...
            const response = await axios.get(`http://localhost:3000/rate?_page=${pageState.paginationModel.page + 1}&_limit=${pageState.paginationModel.pageSize}`);
