...
const Stan = (props) => {
    ...
    const { zgradaid, stanid } = useParams();
    ...
    const zgradaCena = location.state.cenaPoKvadratuBezPDV * location.state.povrsina;
    ...
    function submit(e) {
        ...
        if (data.maticniBroj.length == 13) {
            ...
            //})
            axios.put(`api/DodeliKupca/${stanid}`)
                .then(p => console.log(p))
                .catch(err => console.error(err));
            axios.get(`api/podaciPDF/${stanid}/Cacak/${data.maticniBroj}/${data.mesto}/${data.adresa}`)
                .then(p => {
                    window.location.href = `/Ugovori/${zgradaNaziv}-${broj}.pdf`;
                })
                .catch(err => console.error(err));
        }
        else {
        ...
        <main className="glavni">
          <div className="levo">
            ...
            <div className="desno opis">
                <img className="slike" src="https://www.hidrogradnjapromet.rs/images/projects/ustanicka-buy.jpg" alt="" />
                <h2 >{zgrada.naziv}</h2>
                <h5>{zgrada.ulica}</h5>
                <div>
                    <h2>Stan</h2>
                    <h5>Povrsina: {location.state.povrsina}m2</h5>
                    <h5>Cena po kvdratu: {location.state.cenaPoKvadratuBezPDV}EUR</h5>
                    ...
                    <hr />
                    <h1 className="UC">Ukupna cena: {location.state.cenaPoKvadratuBezPDV * location.state.povrsina}EUR</h1>
